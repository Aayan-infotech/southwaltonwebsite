...
const CombinedComponent = () => {
  ...
  const fetchModels = async () => {
    try {
      const response = await axios.get(`http://13.200.240.28:5006/api/vehicle/`);
  ...
  const removecart = async (id, status) => {
    try {
      ...
      await axios.put(`http://13.200.240.28:5006/api/vehicle/removecart/${id}`, { status });
      fetchModels();
    } catch (error) {
          ...
          return (
            <div key={model.id} className="cart-model">
              <div className="cart-card">
                <img src={`http://13.200.240.28:5006/uploads/${model.image}`} alt={model.vname} className="cart-image" />
